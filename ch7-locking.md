...
如果一次只有一个实例执行，这么实现是没有问题的。但是，如果多个实例同时运行则会出问题。具体来说就是说如果两个 CPU 同时执行 `push`，则两个 CPU 可能都先执行上面代码所示的第 15 行（译者注：即图 7.1 中 CPU 上列的 `l->next = list`），然后再执行第 16 行，从而导致如图 7.2 所示的错误结果。（CPU1 和 CPU2 分别执行第 15 行后）会有两个（新增）链表节点的 `next` 都指向 `list` 先前所指向的 element 对象。同时当第 16 行对 `list` 进行两次赋值时，第二个赋值将覆盖第一个赋值；也就是说第一个赋值中涉及的 element 对象将会丢失。
...
C1/C2 示例中的关键问题在于两个 CPU 获取锁的顺序不同。如果它们都先尝试获取 A，那么其中一个 CPU 就会先获取 A，再获取 B，然后释放它们，这样另一个 CPU 就可以继续执行。更一般地说，锁定代码必须遵循以下规则才能避免死锁：所有持有多个锁的代码路径都必须以相同的顺序获取锁。这种全局锁获取顺序的必要性意味着锁实际上是每个函数编写需要遵循的规范的一部分：调用者必须以一种能够按照约定的顺序获取锁的方式调用函数。
...
自旋锁和中断的共同作用会引发潜在的风险。假设（一个线程执行）`sys_pause` 持有了 `tickslock`，然后一个定时器中断打断了该函数所在线程在当前 CPU 上的执行。`clockintr` 会尝试获取 `tickslock`，因为发现它已经被他人持有，于是中断函数开始进入自旋等待锁被释放。在这种情况下，`tickslock` 永远也得不到释放：因为只有 `sys_pause` 会释放它，但是执行 `sys_pause` 的线程却只有等中断处理函数 `clockintr` 退出后才能继续。于是 CPU 进入死锁状态，并且任何需要获取锁的代码也会因为拿不到锁而被阻塞。
